...
bool addElement(unsigned int start, unsigned int end, std::string &chromosome, std::vector < unsigned int > &starts, std::vector < unsigned int > &ends, std::vector < std::string > &chromosomes, bool first, int distance, int size) {
		if (end - start + 1 < size) {
				return false;
		}
...
DataFrame rcpp_naive(ListOf < ListOf < IntegerVector > > &lengths,
                   ...
                   IntegerVector &chromosomeSizes, int depth, int distance, int size) {
    ...
    for (GenomeIterator iterator (lengths, values, chromosomeSizes); ; iterator.getNext()) {
				if (iterator.hasChangedChromosome() || iterator.isOver()) {
						if (inRegion) {
								unsigned int previousChromosomeId = iterator.getChromosomeId() - 1;
								std::string chromosome = as < std::string >(as< CharacterVector >(chromosomeSizes.names()) [previousChromosomeId]);
								addElement(start, chromosomeSizes[previousChromosomeId], chromosome, starts, ends, chromosomes, first, distance, size);
						}
						inRegion = false;
						first    = true;
						if (iterator.isOver()) {
				...
				if (iterator.getValues().sum() >= depth) {
						...
						if (addElement(start, iterator.getPosition(), chromosome, starts, ends, chromosomes, first, distance, size)) {
								first = false;
						}
						inRegion = false;
				}
