...
runSlice <- function(object) {
    ...
    ranges  <- rcpp_slice(lengths, values, object@chromosomeSizes, object@minDepth, object@minSize, object@maxSize, object@minDifferences)
...
runAllSlice <- function(object) {
    ...
    if (length(intervals) > 0) names(intervals) <- paste("slice", seq(length(intervals)), sep="_")
