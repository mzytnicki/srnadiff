...
opts_chunk$set(message=FALSE, error=FALSE, warning=FALSE,
               tidy=FALSE, cache=TRUE, fig.width=8, fig.height=5)
#opts_chunk$set(message = FALSE, error = FALSE, warning = FALSE,
...
This vignette explains the basics of using `r Biocpkg("srnadiff")` by showing an
...
use [@srnadiff] to cite `r Biocpkg("srnadiff")` and the overall approach
...
* *Data preparation:* In this first step, a convenient *R* object of
...
paths to the BAM files, a `data.frame` with sample and experimental
design information and optionally annotated regions as a `GRanges`
object.
* *Performing srnadiff:* Using the object created in the first step
the user can perform `srnadiff` to find potential DERs and quantify the
statistic signification of these.
* *Visualization of the results:* The DERs obtained in the second
...
```{r, loadLibrary, eval=TRUE, message=FALSE, warning=FALSE}
library(srnadiff)
```
...
* *paths to the BAM files:* a vector with the full paths to the sample BAM
files.
* *sample information:* a `data.frame` with three columns labelled
...
one sample.
* *annotation:* optionally annotation information. A `r Biocpkg("GRanges")`
...
bamFiles <- paste(file.path(basedir, sampleInfo$FileName), "bam",
                  sep = ".")
...
bamFiles   <- paste(file.path(basedir, sampleInfo$FileName),
                    "bam", sep=".")
srnaExp    <- srnadiffExp(bamFiles, sampleInfo, annotReg)
```
...
DERs: the *annotation*, *naive*, *hmm* and *slice* method
(see bellow).
...
vector, it specify whether to skip a method, one of `annotation`,
`naive`, `hmm`, `slice` or combinations thereof.
The default `none`, no method is skipped.
...
```{r, eval=FALSE}
#modify here
plotRegions(srnaExp, regions(srnaExp)[1], type = "p")
```
...
```{r, eval=FALSE}
#modify here
plotRegions(srnaExp, regions(srnaExp)[1], type = "l")
```

`b`: combination of dot and lines plot \medskip

```{r, eval=FALSE}
#modify here
plotRegions(srnaExp, regions(srnaExp)[1], type = "b")
```
...
```{r, eval=FALSE}
#modify here
plotRegions(srnaExp, regions(srnaExp)[1], type = "a")
```
...
```{r, read_whole_genome, eval=FALSE}
#modify here
...
```{r, read_mirbase_mature, eval=FALSE}
#modify here
...
```{r, eval=FALSE}
parameters(srnaExp) <- list(noDiffToDiff = 0.01,
...
The minimun sliding threshold, `minLogFC`, used in the Slice method can
...
from the HMM and the Slice methods), and thus can be counted twice for the
...
```{r, general_parameters, eval=FALSE}
...
exp <- setStrategies(exp, annotation = TRUE, hmm = TRUE, ir = TRUE)
```

where `annotation`, `hmm` and `ir` are logical variables, set to
