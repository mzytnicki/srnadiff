...
- annotation-based,
- irreducible regions of the log-fold change (IR),
- HMM.
...
### Irreducible Regions of the log-fold change
...
condition.
 * The log2 fold change of the mean distribution is computed.
 * The irreducible regions above a given threshold are computed.
 * These putative regions are the output of the strategy.

The irreducible regions are genomic intervals, such that, when split in two
(wherever the split point), the average of the log-fold change of each
sub-region is still above the threshold.
The method has been published by [@rnaseqmap].
